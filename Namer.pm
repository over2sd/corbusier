...
sub genname {
    ...
    my @pc = qw( war long stone ren still min gar dry win gan stan ol bon bone tin kon dar may small hon le lan bay bow blow  bare sign bel sky mer van pan un lun don yun yon ton tan red bed in on an pur per par bur hon hil sil mil fil ful yor tor gol gil far lor lon tar main old cold kin ker new );
    my @pc2 =  qw( field shore way pole well pin shell flow elm pine finth pod oak flame shoe sow mount fast rock run farm fan cor cen cer shoal mait set pul fir fur for lon lun or heel hole haul hall hail don chief ton court );
    my @parts;
    my @parts2;
    for ($syls) {
        if (/a/) { @parts = @pa; }
